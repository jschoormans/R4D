...
% 2014-2016 Martin Uecker <uecker@med.uni-goettingen.de>
	...
	if ispc
		...
		% Paul added print cmd to track bart commands
		fullcmd = [bart_path, filesep, 'bart ', cmd, ' ', in_str, ' ', out_str];
		fprintf('BART start  : [%s]\n',fullcmd)
		ERR = system(fullcmd);
		if ERR~0
			fprintf(2,'BART status : %i\n',ERR)
		else
			fprintf(1,'BART status : %i\n',ERR)
		end
	end
