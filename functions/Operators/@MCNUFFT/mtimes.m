function ress = mtimes(a,bb)
 ...
 if a.adjoint,
     ...
     for tt=1:size(bb,4),
         ...
         %ress1(:,:,tt)=sqrt(sum(abs(res(:,:,:,tt)).^2,3)); %SOS
         %ress3(:,:,tt)=sqrt(sum(abs(res(:,:,:,tt)).*conj(a.b1),3)); %#ok<AGROW>
         ress(:,:,tt)=(sum(res(:,:,:,tt).*(a.b1),3)./sum(abs((a.b1)).^2,3)); %#ok<AGROW>
         %ress(:,:,tt)=sum(res(:,:,:,tt).*(a.b1),3); %#ok<AGROW>
         %ress(:,:,tt)=sum(res(:,:,:,tt).*(a.b1),3); %#ok<AGROW>
     end
 else
     ...
     for ch=1:size(a.b1,3),
        %res=bb(:,:,tt).*conj(a.b1(:,:,ch)); %changed to compex conjugate
        %ress(:,:,ch,tt) = reshape(nufft(res,a.st{tt})/sqrt(prod(a.imSize)),a.dataSize(1),a.dataSize(2)).*a.w(:,:,tt);
        
        res=bb(:,:,tt).*conj(a.b1(:,:,ch)); %#ok<AGROW>
        ress(:,:,ch,tt) = reshape(nufft(res,a.st{tt})/sqrt(prod(a.imSize)),a.dataSize(1),a.dataSize(2)).*a.w(:,:,tt); %NOTE NOT SURE ABOUT W HERE
